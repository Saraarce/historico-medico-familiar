...
export default function MemberProfile({ 
  ...
  const handleDeleteItem = async (storeName: "consultations" | "exams" | "vitals" | "vaccines", id: string) => {
    let confirmed = false;
    try {
      confirmed = confirm("Tem certeza que deseja remover esta informação? Esta ação não pode ser desfeita.");
    } catch (e) {
      console.warn("confirm is blocked by iframe policies, bypass security check.", e);
      confirmed = true;
    }
    if (!confirmed) return;
    try {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden" id={`profile-card-${member.id}`}>
      {/* Member Header */}
      <div className="bg-slate-50 p-6 flex flex-col sm:flex-row items-center gap-5 border-b border-gray-100">
        <div className={`w-16 h-16 rounded-2xl ${member.avatarColor} text-white flex items-center justify-center font-extrabold text-2xl shadow-sm uppercase shrink-0`}>
          {member.name.substring(0, 2)}
        </div>
        
        <div className="text-center sm:text-left flex-1 min-w-0">
          <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2.5 w-full">
            <h1 className="text-2xl font-black text-gray-900 tracking-tight truncate">{member.name}</h1>
            ...
            <button
              ...
              className="py-1.5 px-3 bg-white hover:bg-slate-100 text-blue-600 font-extrabold border border-slate-200 hover:border-slate-300 rounded-xl text-xxs flex items-center gap-1.5 shadow-3xs transition-all cursor-pointer sm:ml-auto ml-2 shrink-0 select-none"
            >
            ...
            <button
              ...
              className="py-1.5 px-3 bg-emerald-50 hover:bg-emerald-100/85 text-emerald-700 font-extrabold border border-emerald-200 hover:border-emerald-300 rounded-xl text-xxs flex items-center gap-1.5 shadow-3xs transition-all cursor-pointer shrink-0 select-none"
            >
          ...
          <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2 mt-2 text-xs text-gray-500 font-medium select-none">
            <div className="bg-slate-100 border border-slate-250/20 px-2.5 py-1 rounded-lg flex items-center gap-1.5 shrink-0">
            ...
            <div className="text-amber-700 bg-amber-50/70 px-2.5 py-1 rounded-lg border border-amber-100 flex items-center gap-1 shrink-0">
              <AlertCircle className="w-3.5 h-3.5 text-amber-500" />
              <span>Alergias: <span className="font-bold">{member.allergies || "Nenhuma"}</span></span>
            </div>
          </div>

          {/* Comorbidades e Medicamentos em Uso */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-4 pt-3.5 border-t border-slate-200/50">
          ...
          {(["summary", "consultations", "surgeries", "exams", "vaccines"] as ProfileTab[]).map((tab) => {
            ...
            return (
              <button
                key={tab}
                type="button"
                className={`flex-1 min-w-[100px] sm:min-w-0 py-3 sm:py-4 px-2 flex items-center justify-center gap-1.5 text-xs sm:text-sm font-bold border-b-2 whitespace-nowrap transition-colors cursor-pointer ${
                  isSelected 
          ...
          <div className="space-y-6">
            ...
            <div className="flex justify-between items-center">
              ...
              <button
                ...
                className="py-2 px-3 bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs rounded-xl flex items-center gap-1.5 shadow-xs transition-colors cursor-pointer"
              >
